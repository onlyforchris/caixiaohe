...
- 保护用户数据：config.json / invoice_ledger.json / used_ledger.json / financekit.db / python_path.txt / .venv / backups 等绝不覆盖；
...
LATEST_RELEASE = "https://github.com/onlyforchris/caixiaohe/releases/latest"
TAG_ZIP = "https://github.com/onlyforchris/caixiaohe/archive/refs/tags/%s.zip"
PROTECT_FILES = {"config.json", "invoice_ledger.json", "used_ledger.json", "python_path.txt", "financekit.db"}
PROTECT_DIRS = {".venv", "__pycache__", ".git", "backups"}
...
def protected(rel):
    p = rel.replace("\\", "/")
    parts = p.split("/")
    return any(x in PROTECT_DIRS for x in parts) or (
        len(parts) == 1 and (p in PROTECT_FILES or p.startswith("financekit.db-"))
    )


def fetch(url, timeout=180):
    req = urllib.request.Request(url, headers={"User-Agent": "caixiaohe-updater"})
    with urllib.request.urlopen(req, timeout=timeout) as r:
        return r.read()


def latest_tag(timeout=60):
    req = urllib.request.Request(LATEST_RELEASE, headers={"User-Agent": "caixiaohe-updater"})
    with urllib.request.urlopen(req, timeout=timeout) as r:
        final_url = r.geturl()
...
def collect(zf):
    ...
    for info in zf.infolist():
        ...
        if protected(rel):
            continue
        blob = zf.read(info)
        if rel.replace("\\", "/") == "financekit/paths.py":
...
def main():
    ...
    if "--url" in args:
        ...
        if i + 1 >= len(args):
            ...
            return 1
        override = args[i + 1]
    local_path = os.path.join(HERE, "financekit", "paths.py")
